package com.example.money_manager_app.fragment.lock_screen
...
import com.example.money_manager_app.databinding.FragmentPasscodeBinding
...
class PasscodeFragment : Fragment() {
    ...
    private fun setUpNumberActionButton() {
        binding.numericKeyboard.btnConfirm.setOnClickListener {
            if (checkIfPasscodeExists()) {
                checkPasscode().let {
                    if (it) {
                        findNavController().navigate(R.id.action_passcodeFragment_to_homeFragment)
                    } else {
                ...
                findNavController().navigate(R.id.action_passcodeFragment_to_confirmPasscodeFragment)
            }
        }
    }
    ...
    private fun checkPasscode(): Boolean {
        ...
        val savedPasscode = activity?.getSharedPreferences(PASSCODE_PREF, Context.MODE_PRIVATE)
    ...
    private fun checkIfPasscodeExists(): Boolean {
        val sharedPreferences = activity?.getSharedPreferences(PASSCODE_PREF, Context.MODE_PRIVATE)
